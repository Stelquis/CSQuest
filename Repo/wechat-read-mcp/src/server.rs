...
impl WeixinServer {
    /// 读取微信公众号文章内容
    #[tool(description = "读取微信公众号文章内容。接收一个微信文章 URL，提取结构化内容（标题、作者、正文 Markdown、图片列表）。URL 必须以 https://mp.weixin.qq.com/s/ 开头。返回：title, author, publish_time, content(纯文本), content_markdown(带格式), images(图片URL列表)。")]
